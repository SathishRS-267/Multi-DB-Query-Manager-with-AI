...
#     username: str
#     password: str
#     dbms: str = "postgresql"  # Default to PostgreSQL
...
#     def create_connection(self, conn_details: ConnectionRequest) -> str:
#         """Create a new database connection and return connection ID"""
#         connection_id = f"{conn_details.username}@{conn_details.host}/{conn_details.database}"
        
#         try:
#             if conn_details.dbms == "postgresql":
#                 # Create SQLAlchemy engine
#                 connection_string = f"postgresql://{conn_details.username}:{conn_details.password}@{conn_details.host}:{conn_details.port}/{conn_details.database}"
#                 engine = create_engine(connection_string)
#                 conn = engine.connect()
                
#                 # Store connection and engine
#                 self.connections[connection_id] = conn
#                 self.engines[connection_id] = engine
                
#                 # Collect metadata about the database
#                 self.metadata[connection_id] = self._get_db_metadata(engine)
                
#                 return connection_id
#             else:
#                 raise ValueError(f"Unsupported DBMS: {conn_details.dbms}")
                
#         except Exception as e:
#             logger.error(f"Connection error: {str(e)}")
#             raise HTTPException(status_code=400, detail=f"Connection failed: {str(e)}")
    
#     def _get_db_metadata(self, engine):
